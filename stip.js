...
(function(window,$,undefined){
	...
	var Tip = function( type, dom, ops ){
		...
		tpl = '<div class="{wraperCss} {arrowClass}"><i class="arrowOut"></i><i class="arrowIn"></i><div class="{contentCss}"></div></div>',
		slfIdCount = 0,
		$iframe = null,
		//默认配置
		Config = {
	...
	Tip.prototype = {
			...
			if(this.conf.tplSelector){ //指定页面中已存在的tip
				...
				if(cache[id]){
					...
					$tip = $(tpl.replace('{wraperCss}', this.conf.wraperCss)
						...
						.replace('{arrowClass}', 'u-pop-loc-'+this.conf.loc))
				}
			}
			if($tip && $tip[0]){
		...
		show : function( dom ){
			...
			if(this.conf.beforeShowFn){
				...
				msg = this.conf.msg || $tar.attr('msg') || $tar.attr('info') ||  $tar.attr('title') ||  $tar.attr('data-msg') ||  $tar.attr('data-info') ||  $tar.attr('data-title');
				$('.'+this.conf.contentCss, $tip).html(msg);
			}
			$tip.width( 'auto' );
			maxWidth = $tar.attr('data-maxWidth') ||this.conf.maxWidth;
			minWidth = $tar.attr('data-minWidth')||this.conf.minWidth;
			width = $tar.attr('data-width')||this.conf.width;
			if(maxWidth || minWidth){
				if(maxWidth && $tip.outerWidth() > maxWidth){
					$tip.width( +maxWidth );
				}else if(minWidth && $tip.outerWidth() < minWidth){
					$tip.width( +minWidth );
				}else{
					$tip.width( 'auto' );
				}
